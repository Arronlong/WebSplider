...
module.exports = function(cid, obj) {
    ...
    function getTime() {
        //数据成功更新,则下次更新在1天后，否则1分钟后继续。
        //当连续五次自动更新失败，说明目标网站可能出现问题，则不自动进行数据更新，只能等到用户下次调用该数据接口时，再进行定时器初始化
        if (state === 1) {
            return new Date(time.getFullYear(), time.getMonth(), time.getDate(), time.getHours(), time.getMinutes() + 1, time.getSeconds());
        } else if (state === 2) {
            return new Date(time.getFullYear(), time.getMonth(), time.getDate(), time.getHours(), time.getMinutes(), time.getSeconds() + 30);
        } else if (state === 3) {
